...
# -*- coding: utf-8 -*-
"""
02_データ取得.py（DB連携版・keywords_raw保存対応 / config.py運用 / STA-END方式）
...
TABLE_NAME = str(getattr(CFG, "TABLE_NAME", "items") or "items")
BASE_OUTPUT_ROOT = Path(getattr(CFG, "BASE_OUTPUT_ROOT"))

# --- SQLite（安定稼働用） ---
BUSY_TIMEOUT_MS = int(getattr(CFG, "BUSY_TIMEOUT_MS", 60000))
SQLITE_JOURNAL_MODE = str(getattr(CFG, "SQLITE_JOURNAL_MODE", "WAL")).strip() or "WAL"
SQLITE_SYNCHRONOUS = str(getattr(CFG, "SQLITE_SYNCHRONOUS", "NORMAL")).strip() or "NORMAL"
LOCK_RETRY_MAX = int(getattr(CFG, "LOCK_RETRY_MAX", 25))
LOCK_RETRY_SLEEP_SEC = float(getattr(CFG, "LOCK_RETRY_SLEEP_SEC", 0.8))

# --- STA/END ---
STA_02 = int(getattr(CFG, "STA_02", 1))
END_02 = int(getattr(CFG, "END_02", 2))

# --- 共通：ピック順 ---
# 基本は共通の PICK_ORDER を使う（必要なら PICK_ORDER_02 で上書き）
PICK_ORDER_02 = (getattr(CFG, "PICK_ORDER_02", "") or "").strip()
if not PICK_ORDER_02:
    PICK_ORDER_02 = (getattr(CFG, "PICK_ORDER", "post_date_desc") or "post_date_desc").strip()

# --- 02固有：取得系 ---
MAX_COMMENTS_TO_FETCH = int(getattr(CFG, "MAX_COMMENTS_TO_FETCH", 75))
MAX_CONSECUTIVE_MISSES = int(getattr(CFG, "MAX_CONSECUTIVE_MISSES", 20))

HEADLESS_MODE = bool(getattr(CFG, "HEADLESS_MODE", True))
WAIT_TIMEOUT = int(getattr(CFG, "WAIT_TIMEOUT_MS", 45000))  # ms
REQUEST_INTERVAL_MS = int(getattr(CFG, "REQUEST_INTERVAL_MS", 300))
USER_AGENT = str(
    getattr(
        CFG,
        "USER_AGENT",
...
SAVE_DEBUG_ON_PARSE_FAIL = bool(getattr(CFG, "SAVE_DEBUG_ON_PARSE_FAIL", True))
ENABLE_RELATED_KEYWORDS = bool(getattr(CFG, "ENABLE_RELATED_KEYWORDS", True))

# フォルダ名タイトルの最大文字数
FOLDER_TITLE_MAX_CHARS = int(getattr(CFG, "FOLDER_TITLE_MAX_CHARS", 10))

# 投稿しない判定（運用でON/OFFしたくなるのでconfig化）
ENABLE_DEPLOY_SKIP_IF_SHORTAGE = bool(getattr(CFG, "ENABLE_DEPLOY_SKIP_IF_SHORTAGE", True))
REQUIRE_TOTAL_N = int(getattr(CFG, "REQUIRE_TOTAL_N", 15))
REQUIRE_RATIO_N = int(getattr(CFG, "REQUIRE_RATIO_N", 15))

# NGワード
ENABLE_EXCLUDE_BADWORDS = bool(getattr(CFG, "ENABLE_EXCLUDE_BADWORDS", True))
BADWORDS_DEFAULT = [
    "殺","死","亡","自殺","他殺","事故死","ころす","殺す","死ね","氏ね","しぬ","ﾀﾋ","タヒ",
    "レイプ","ﾚｲﾌﾟ","売春",
    "朝鮮",
    "ガルちゃん","ｶﾞﾙちゃん","ガル民","ｶﾞﾙ民",
]

_BADWORDS_CFG = getattr(CFG, "BADWORDS", None)
if isinstance(_BADWORDS_CFG, str):
    BADWORDS = [w.strip() for w in _BADWORDS_CFG.split(",") if w.strip()]
elif _BADWORDS_CFG:
    BADWORDS = list(_BADWORDS_CFG)
else:
    BADWORDS = BADWORDS_DEFAULT

BADWORDS_NORMALIZE = bool(getattr(CFG, "BADWORDS_NORMALIZE", True))
...
# メイン画像（固定）
MAIN_IMAGE_XPATH = '//*[@id="topic"]/div[2]/div[1]//img'
MAIN_IMAGE_FILENAME = "main.jpg"
MAIN_DIR_NAME = "main"

# 関連キーワード
RELATED_KEYWORDS_CSS = "#related-words a"
ENABLE_RELATED_KEYWORDS = bool(ENABLE_RELATED_KEYWORDS)

# 保存用 placeholder
IMAGE_PLACEHOLDER_DIR = "image/comment"
IMAGE_PLACEHOLDER_EXT = ".png"

# debug
SAVE_DEBUG_ON_PARSE_FAIL = bool(SAVE_DEBUG_ON_PARSE_FAIL)
