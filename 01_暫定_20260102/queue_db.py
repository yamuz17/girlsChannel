...
def load_env(env_path: Optional[Path] = None) -> None:
    """
    .env を読み込んで os.environ に反映する。
    env_path を渡した場合はそのファイルを優先する。
    """
...
def build_queue_config_from_env() -> QueueConfig:
    ...
    db_path = _env_required_path("DB_PATH")
    base_output_root = _env_required_path("BASE_OUTPUT_ROOT")

    table = _env_str("TABLE_NAME", "items").strip() or "items"

    busy = _env_int("BUSY_TIMEOUT_MS", 60000)
    wal = _env_bool("SQLITE_WAL", True)
    sync = _env_str("SQLITE_SYNCHRONOUS", "NORMAL").strip().upper() or "NORMAL"

    enable_idx = _env_bool("ENABLE_PICK_QUEUE_INDEX", True)
    idx_name = _env_str("PICK_QUEUE_INDEX_NAME", "idx_items_pick_queue").strip() or "idx_items_pick_queue"
