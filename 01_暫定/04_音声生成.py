...
# env load
# =========================
SCRIPT_DIR = Path(__file__).resolve().parent  # 互換のため残す（未使用）
...
STA_04 = int(getattr(CFG, "STA_04", 3))
END_04 = int(getattr(CFG, "END_04", 4))

ENGINE_URL = (getattr(CFG, "ENGINE_URL", "http://127.0.0.1:50021") or "").strip() or "http://127.0.0.1:50021"
VOICE_BOOT_TIMEOUT_SEC = int(getattr(CFG, "VOICE_BOOT_TIMEOUT_SEC", 60))
VOICE_POLL_INTERVAL_SEC = int(getattr(CFG, "VOICE_POLL_INTERVAL_SEC", 2))

TOTAL_VIDEO_SEC = float(getattr(CFG, "TOTAL_VIDEO_SEC", 45.0))
SILENCE_MS = int(getattr(CFG, "SILENCE_MS", 400))
MIN_SEC_PER_COMMENT = float(getattr(CFG, "MIN_SEC_PER_COMMENT", 2.0))
MAX_SEC_PER_COMMENT = float(getattr(CFG, "MAX_SEC_PER_COMMENT", 7.5))
...
def main() -> int:
    ...
    print(f"[INFO] table={QCFG.table} STA_04={STA_04} END_04={END_04} order={PICK_ORDER}")
    ...
    try:
        with queue_db.connect_db(QCFG) as con:
            ...
            picked = queue_db.pick_one(con, QCFG.table, STA_04, PICK_ORDER)
            if picked is None:
            ...
            try:
                ...
                queue_db.mark_done(con, QCFG.table, item_id, STA_04, END_04)
                print(f"[OK] done. check_create {STA_04} -> {END_04} (id={item_id})")
                return 0

            except Exception as e:
                err = f"{type(e).__name__}: {e}"
                queue_db.mark_fail(con, QCFG.table, item_id, STA_04, err)
