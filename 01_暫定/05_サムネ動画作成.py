...
STA_05 = int(getattr(CFG, "STA_05", 4))
END_05 = int(getattr(CFG, "END_05", 5))
...
# タイトル（見た目）
TITLE_STYLE_PRESET = int(int(getattr(CFG, "TITLE_STYLE_PRESET", 1)))
TITLE_ACCENT_WORDS = [w.strip() for w in ((getattr(CFG, "TITLE_ACCENT_WORDS", "登録") or "") or "").split(",") if w.strip()]
...
# イントロ音源
START_DIR = Path(str(getattr(CFG, "START_DIR", CFG.START_DIR))).expanduser()
START_MP3_NAME = (getattr(CFG, "START_MP3_NAME", CFG.START_MP3_NAME) or "").strip()

# intro movie settings
INTRO_SEC = float(((getattr(CFG, "INTRO_SEC", "0.9") or "") or "0.9").strip())
INTRO_FPS = int(int(getattr(CFG, "INTRO_FPS", 30)))
INTRO_AUDIO_SR = int(int(getattr(CFG, "INTRO_AUDIO_SR", 48000)))
INTRO_AUDIO_CH = int(int(getattr(CFG, "INTRO_AUDIO_CH", 2)))
INTRO_AUDIO_BITRATE = ((getattr(CFG, "INTRO_AUDIO_BITRATE", "192k") or "") or "192k").strip()
...
def main() -> int:
    ...
    print(f"[INFO] table={QCFG.table} STA_05={STA_05} END_05={END_05} order={PICK_ORDER}")
    ...
    with queue_db.connect_db(QCFG) as con:
        ...
        picked = queue_db.pick_one(con, QCFG.table, STA_05, PICK_ORDER)
        if picked is None:
        ...
        try:
            ...
            queue_db.mark_done(con, QCFG.table, item_id, STA_05, END_05)
            print(f"[OK] done. check_create {STA_05} -> {END_05} (id={item_id})")
            return 0

        except Exception as e:
            err = f"{type(e).__name__}: {e}"
            queue_db.mark_fail(con, QCFG.table, item_id, STA_05, err)
