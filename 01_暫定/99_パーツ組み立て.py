...
SQLITE_JOURNAL_MODE = (str(getattr(CFG, "SQLITE_JOURNAL_MODE", "WAL")) or "WAL").strip() or "WAL"
SQLITE_SYNCHRONOUS = (str(getattr(CFG, "SQLITE_SYNCHRONOUS", "NORMAL")) or "NORMAL").strip() or "NORMAL"
...
# --- 共通：ステージ運用（STA/END方式） ---
STA_99 = int(getattr(CFG, "STA_99", 5))
END_99 = int(getattr(CFG, "END_99", 6))

# --- 共通：ピック順 ---
PICK_ORDER_99 = (getattr(CFG, "PICK_ORDER_99", "") or "").strip()
if not PICK_ORDER_99:
    PICK_ORDER_99 = (getattr(CFG, "PICK_ORDER", "post_date_desc") or "post_date_desc").strip()

# --- 99（動画組み立て）固有 ---
FPS = int(getattr(CFG, "FPS", 30))
AUTO_BUILD_AUDIO_DESC = bool(getattr(CFG, "AUTO_BUILD_AUDIO_DESC", True))
WRITE_TO_LOCAL_TMP = bool(getattr(CFG, "WRITE_TO_LOCAL_TMP", True))
ENABLE_FASTSTART = bool(getattr(CFG, "ENABLE_FASTSTART", False))

# tmp掃除
CLEANUP_TMP = bool(getattr(CFG, "CLEANUP_TMP", True))

# duration下限（短すぎるとffmpegが不安定になるのを避ける）
MIN_SEG_SEC = float(getattr(CFG, "MIN_SEG_SEC", 0.05))  # 0.05〜0.10 推奨

# Preview（冒頭に付ける）
ENABLE_PREVIEW = bool(getattr(CFG, "ENABLE_PREVIEW", True))
PREVIEW_REL = Path(str(getattr(CFG, "PREVIEW_REL", "image/preview/preview.mp4")))
PREVIEW_REQUIRED = bool(getattr(CFG, "PREVIEW_REQUIRED", True))

# Ending（最後に付ける締め）
ENABLE_ENDING = bool(getattr(CFG, "ENABLE_ENDING", True))
ENDING_IMAGE_PATH = Path(str(getattr(CFG, "ENDING_IMAGE_PATH", CFG.ENDING_IMAGE_PATH)))
ENDING_AUDIO_PATH = Path(str(getattr(CFG, "ENDING_AUDIO_PATH", CFG.ENDING_AUDIO_PATH)))
ENDING_PAD_SEC = float(getattr(CFG, "ENDING_PAD_SEC", 0.0))

# BGM
ENABLE_BGM = bool(getattr(CFG, "ENABLE_BGM", True))
BGM_ODD_PATH = Path(str(getattr(CFG, "BGM_ODD_PATH", CFG.BGM_ODD_PATH)))
BGM_EVEN_PATH = Path(str(getattr(CFG, "BGM_EVEN_PATH", CFG.BGM_EVEN_PATH)))
BGM_VOLUME = float(getattr(CFG, "BGM_VOLUME", 0.12))
BGM_DUCKING = bool(getattr(CFG, "BGM_DUCKING", True))
BGM_FADE_SEC = float(getattr(CFG, "BGM_FADE_SEC", 0.30))
BGM_START_SEC_ODD = float(getattr(CFG, "BGM_START_SEC_ODD", 0.0))
BGM_START_SEC_EVEN = float(getattr(CFG, "BGM_START_SEC_EVEN", 0.0))

# concat時「音声無しmp4」が混じっても落ちないようにする
ENSURE_AUDIO_FOR_CONCAT = bool(getattr(CFG, "ENSURE_AUDIO_FOR_CONCAT", True))
SILENCE_SR = int(getattr(CFG, "SILENCE_SR", 48000))
SILENCE_CH = int(getattr(CFG, "SILENCE_CH", 2))
SILENCE_BITRATE = (str(getattr(CFG, "SILENCE_BITRATE", "192k")) or "192k").strip() or "192k"
