...
# --- pick高速化インデックス（任意） ---
ENABLE_PICK_QUEUE_INDEX = getattr(CFG, "ENABLE_PICK_QUEUE_INDEX", True)
PICK_QUEUE_INDEX_NAME = getattr(CFG, "PICK_QUEUE_INDEX_NAME", "idx_items_pick_queue")
PICK_QUEUE_INDEX_SQL = f"{TABLE_NAME}(check_create, id DESC)"  # 壊しにくい最小構成

# --- scripts ---
SCRIPTS_DIR = CFG.SCRIPTS_DIR

SCRIPT_02 = SCRIPTS_DIR / getattr(CFG, "SCRIPT_02_NAME", "02_データ取得.py")
SCRIPT_03 = SCRIPTS_DIR / getattr(CFG, "SCRIPT_03_NAME", "03_画像生成.py")
SCRIPT_04 = SCRIPTS_DIR / getattr(CFG, "SCRIPT_04_NAME", "04_音声生成.py")
SCRIPT_05 = SCRIPTS_DIR / getattr(CFG, "SCRIPT_05_NAME", "05_サムネ動画作成.py")
SCRIPT_99 = SCRIPTS_DIR / getattr(CFG, "SCRIPT_99_NAME", "99_パーツ組み立て.py")

# --- launcher behavior ---
RUNS_DEFAULT = int(CFG.RUNS_DEFAULT)
STOP_ON_ERROR = bool(CFG.STOP_ON_ERROR)
RESET_TO_ZERO_ON_FAIL_02 = bool(getattr(CFG, "RESET_TO_ZERO_ON_FAIL_02", False))

# 対象が見つからない（キュー空）時に待つ秒数（空のときだけ sleep する）
SLEEP_SEC_WHEN_EMPTY = float(getattr(CFG, "SLEEP_SEC_WHEN_EMPTY", 0))

# 05に folder_name を渡す互換運用（旧05を残す場合用）
PASS_FOLDER_NAME_TO_05 = bool(getattr(CFG, "PASS_FOLDER_NAME_TO_05", False))

# --- STA/END ---
STA_02, END_02 = int(CFG.STA_02), int(CFG.END_02)
STA_03, END_03 = int(CFG.STA_03), int(CFG.END_03)
STA_04, END_04 = int(CFG.STA_04), int(CFG.END_04)
STA_05, END_05 = int(CFG.STA_05), int(CFG.END_05)
STA_99, END_99 = int(CFG.STA_99), int(CFG.END_99)

# --- タイムアウト（秒） ---
TIMEOUT_02 = getattr(CFG, "TIMEOUT_02", None)
TIMEOUT_03 = getattr(CFG, "TIMEOUT_03", None)
TIMEOUT_04 = getattr(CFG, "TIMEOUT_04", None)
TIMEOUT_05 = getattr(CFG, "TIMEOUT_05", None)
TIMEOUT_99 = getattr(CFG, "TIMEOUT_99", None)
...
def _guard_stage_chain_or_exit() -> None:
    """
    このランチャーのロジックは「END_x == 次の STA_y」を前提としている。
    ここがズレると次工程が実行されず、意図せずスキップするので起動時に止める。
    """
    ok = (
        END_02 == STA_03
        and END_03 == STA_04
    ...
    if not ok:
        msg = (
            "[ERROR] STA/END chain mismatch.\n"
            f"  END_02={END_02} must equal STA_03={STA_03}\n"
            f"  END_03={END_03} must equal STA_04={STA_04}\n"
            f"  END_04={END_04} must equal STA_05={STA_05}\n"
            f"  END_05={END_05} must equal STA_99={STA_99}\n"
            "Fix config.py so each END matches the next STA."
        )
        raise SystemExit(msg)


def main() -> int:
    _guard_stage_chain_or_exit()
    ...
    banner(f"LAUNCHER END  {now_jst_str()}  ok={ok_count} err={err_count} empty={empty_count}")
