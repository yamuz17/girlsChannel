...
CREATE TABLE IF NOT EXISTS {TABLE_ALL} (
  ...
  comment_average REAL                      -- comments_count / minutes(last_post-first_post)
);
...
#   - PKは id（ユーザー指定）
#   - do: created_at/updated_at は無し（ユーザー指定）
#   - priority 追加（ユーザー指定）
#
...
CREATE TABLE IF NOT EXISTS {TABLE_DO} (
  ...
  code TEXT NOT NULL UNIQUE,
  title TEXT NOT NULL,
  priority INTEGER NOT NULL DEFAULT 0,    -- ★追加: 1が最優先（同期で再計算）
...
def ensure_schema(con: sqlite3.Connection) -> None:
    ...
    4) 追加カラム（comment_average / priority）が旧DBに無ければ追加
    """
    # --- 旧スキーマ判定（items_all） ---
    if _table_exists(con, TABLE_ALL):
        ...
        if ("code" not in colset) and ("id" in colset):
            ...
            # comment_average は後で再計算するので NULL のまま
            con.execute(
                f\"\"
                INSERT INTO {tmp} (
                  code, check_date, first_seen_at, first_post, last_post,
                  comments_count, category, title, out_auto, out_manual, excluded, comment_average
                )
                SELECT
                  id AS code,
               ...
               CASE
                 ...
                 ELSE
                   CASE
                     ...
                     ELSE (comments_count * 1.0) / ((julianday(last_post) - julianday(first_post)) * 24.0 * 60.0)
                   END
               END
        \"\"
    )
...
def sync_items_do_from_all(con: sqlite3.Connection) -> None:
    ...
    con.execute(
        ...
        UPDATE {TABLE_DO}
           SET priority = (
             SELECT pr FROM ranked WHERE ranked.code = {TABLE_DO}.code
           )
    ...
    top5 = con.execute(
        f\"\"
        SELECT priority, code, title
          FROM {TABLE_DO}
         ORDER BY priority ASC
         LIMIT 5
        \"\"
    ).fetchall()
    print(f"[SYNC] {TABLE_DO}: upsert_changes={upsert_changes} deleted_excluded={deleted_excluded} deleted_missing={deleted_missing} priority_updated={pr_updated} total={total_do}")
    if top5:
        print("[SYNC] top priorities (first 5):")
        for pr, code, title in top5:
            print(f"  - {pr:>3}  code={code}  title={title[:50]}")
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # STEP 2.5) comment_average 再計算
            recompute_comment_average(con)

            # STEP 3) items_all -> items_do 同期 & priority 付与
    ...
    print(f"  new_inserts={new_inserts} updated={updated_ct}")
    ...
    print(f"  first_post_filled={first_post_filled} failed={first_post_failed} skipped_excl={first_post_skipped_excluded}")
