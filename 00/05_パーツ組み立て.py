...
# === ★最後に付ける「締め」(画像+音声) ===
ENABLE_ENDING = True
ENDING_IMAGE_PATH = Path(r"/Users/yumahama/Library/CloudStorage/GoogleDrive-yuma17.service@gmail.com/マイドライブ/plan_001/image/last_01.png")
ENDING_AUDIO_PATH = Path(r"/Users/yumahama/Library/CloudStorage/GoogleDrive-yuma17.service@gmail.com/マイドライブ/plan_001/last/last.wav")

# 締めを「無音余白」で少しだけ伸ばしたい場合（秒）
ENDING_PAD_SEC = 0.0
...
def concat_main_and_ending(main_mp4: Path, ending_mp4: Path, out_mp4: Path, log_path: Path) -> None:
    # concat filterで確実に繋ぐ（エンコードし直し）
...
def run_build(parent_dir: Path, item_id: int) -> Path:
    ...
    ffmpeg_log_concat = log_dir / "ffmpeg_concat_ending.log"

    tmp_video_main = TMP_DIR / "youtube_upload_tmp_main.mp4"
    tmp_video_final = TMP_DIR / "youtube_upload_tmp_final.mp4"
    tmp_ending_video = TMP_DIR / "youtube_upload_tmp_ending.mp4"
    ...
    # ★まず「本編」を作る（締めを後で結合）
    ffmpeg_out_main = out_mp4
    if WRITE_TO_LOCAL_TMP:
        ffmpeg_out_main = tmp_video_main

    def build_cmd_main(ducking: bool) -> Tuple[List[str], str]:
    ...
    # 1) まずダッキングありで試す
    try:
        cmd1, _ = build_cmd_main(ducking=bool(BGM_DUCKING))
        ...
        if bgm_path and BGM_DUCKING:
            ...
            cmd2, _ = build_cmd_main(ducking=False)
    ...
    if ENABLE_ENDING:
        ...
        # concat結果の出力先
        ffmpeg_out_final = out_mp4
        if WRITE_TO_LOCAL_TMP:
            ffmpeg_out_final = tmp_video_final

        concat_main_and_ending(
            main_mp4=ffmpeg_out_main,
            ending_mp4=tmp_ending_video,
            out_mp4=ffmpeg_out_final,
            log_path=ffmpeg_log_concat,
        )

        # 最終を out_mp4 へ
        if WRITE_TO_LOCAL_TMP:
            safe_copy(ffmpeg_out_final, out_mp4)
    else:
        # 締め無しなら、出来た本編を out_mp4 へ
        if WRITE_TO_LOCAL_TMP:
            safe_copy(ffmpeg_out_main, out_mp4)
    ...
    if ENABLE_ENDING:
        print(f"FFMPEG LOG (ENDING): {ffmpeg_log_ending}")
        print(f"FFMPEG LOG (CONCAT ENDING): {ffmpeg_log_concat}")
